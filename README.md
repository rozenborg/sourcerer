...
Headless content database. A daily job fetches articles + podcast transcripts from configured sources, summarizes them via Claude, and upserts rows into Supabase. Other apps (review UIs, MCP servers, Skills, dashboards) read from Supabase.

- **Source of truth:** Supabase (`articles` table)
- **Schedule:** GitHub Actions cron, 08:00 UTC daily
- **Adding sources:** edit `feeds.yaml`

## Architecture

```
feeds.yaml ──► pull.py ──► fetchers.py ──► Claude (summarize) ──► Supabase.articles
                                                                        │
...
- `OPENAI_API_KEY` — for Whisper transcription (podcasts)
- `SUPABASE_URL`
- `SUPABASE_SERVICE_KEY`

The workflow at [`.github/workflows/daily.yaml`](.github/workflows/daily.yaml) runs daily at 08:00 UTC. You can also trigger it manually via the Actions tab.
...
- **Cost:** Supabase + Cloudflare free tiers cover this workload comfortably. The recurring cost is Anthropic + OpenAI API usage from `pull.py`.
